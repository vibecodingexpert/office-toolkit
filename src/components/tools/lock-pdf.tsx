...
import {
  Upload, Download, FileText, Check, X, FileDown, Lock, KeyRound, Shield,
} from "lucide-react"
...
export function LockPdf() {
  ...
  const [encryptionLevel, setEncryptionLevel] = React.useState("128")
  ...
  }, [fileInfo, password, confirmPassword, encryptionLevel])

  const download = React.useCallback(() => {
    if (!fileInfo?.resultUrl) return
    const a = document.createElement("a")
    a.href = fileInfo.resultUrl
    a.download = fileInfo.file.name.replace(/\.pdf$/i, "") + "_protected.pdf"
    a.click()
  }, [fileInfo])
  ...
  return (
    <Card className="space-y-6 p-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-lg font-semibold">Lock PDF</h2>
          <p className="text-sm text-muted-foreground">Protect PDF files with password encryption</p>
        </div>
      </div>
      ...
      {!fileInfo ? (
        ...
        <div className="space-y-5">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full rounded-xl border border-border bg-background px-4 py-2.5 pl-10 text-sm text-foreground transition-all focus:border-primary/50 focus:outline-none focus:ring-1 focus:ring-primary/30"
                  />
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className={cn(
                    "w-full rounded-xl border bg-background px-4 py-2.5 text-sm text-foreground transition-all focus:outline-none focus:ring-1",
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-foreground">Encryption Level</label>
          ...
          {isProcessing && (
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                ...
                Encrypting PDF with {encryptionLevel}-bit AES...
              </div>
            ...
            >
              <div className="flex items-center justify-between rounded-xl bg-gradient-to-r from-emerald-500/5 to-primary/5 border border-emerald-500/10 p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-foreground">PDF Protected</p>
                    <p className="text-xs text-muted-foreground">{encryptionLevel}-bit AES encryption · {formatSize(fileInfo.resultSize)}</p>
                  </div>
                </div>
          ...
          {fileInfo.status === "idle" && !isProcessing && (
            <Button onClick={process} size="lg" className="w-full" icon={<Lock className="h-4 w-4" />} disabled={confirmPassword.length > 0 && password !== confirmPassword}>
              Lock PDF
            </Button>
          )}
        </div>
