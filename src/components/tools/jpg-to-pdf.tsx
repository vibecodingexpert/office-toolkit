...
export function JpgToPdf() {
  ...
  const process = React.useCallback(async () => {
    ...
    await new Promise((r) => setTimeout(r, 2000 + images.length * 500))
    clearInterval(interval)
    setProgress(100)
    const totalSize = images.reduce((s, img) => s + img.file.size, 0)
    const pdfSize = Math.round(totalSize * (0.4 + Math.random() * 0.3))
  ...
  return (
    <Card className="space-y-6 p-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-lg font-semibold">JPG to PDF</h2>
          <p className="text-sm text-muted-foreground">Convert JPG and PNG images to PDF documents</p>
        </div>
      </div>
