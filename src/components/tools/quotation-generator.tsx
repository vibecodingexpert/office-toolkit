...
export function QuotationGenerator() {
  ...
  const handleDownload = async () => {
    ...
    doc.text(quoteNum, margin, y + 6)
    y += 15

    if (logo) {
      try { doc.addImage(logo, "PNG", margin, y, 30, 15) } catch { }
      y += 20
    }
    ...
    doc.setTextColor(100, 116, 139)
    if (company.address) { y += 5; doc.text(company.address, margin, y) }
    if (company.email) { y += 5; doc.text(company.email, margin, y) }
    if (company.phone) { y += 5; doc.text(company.phone, margin, y) }
    y += 10
    ...
    if (client.address) { y += 5; doc.text(client.address, margin, y) }
    if (client.email) { y += 5; doc.text(client.email, margin, y) }
    if (client.phone) { y += 5; doc.text(client.phone, margin, y) }

    doc.setFont("helvetica", "bold")
    doc.setTextColor(26, 26, 46)
    doc.text("Date:", pw - margin - 30, margin + 15)
    doc.setFont("helvetica", "normal")
    doc.setTextColor(100, 116, 139)
    doc.text(formatDate(new Date(date)), pw - margin, margin + 15, { align: "right" })
    doc.text("Valid Until:", pw - margin - 30, margin + 21)
    doc.text(validUntil ? formatDate(new Date(validUntil)) : "N/A", pw - margin, margin + 21, { align: "right" })

    y = Math.max(y + 10, 80)
    const tableTop = y
    doc.setFillColor(240, 253, 244)
    doc.rect(margin, tableTop - 4, pw - 2 * margin, 8, "F")
    doc.setFont("helvetica", "bold")
    doc.setFontSize(8)
    doc.setTextColor(5, 150, 105)
    doc.text("DESCRIPTION", margin + 2, tableTop)
    doc.text("QTY", pw - margin - 55, tableTop)
    doc.text("RATE", pw - margin - 35, tableTop)
    doc.text("AMOUNT", pw - margin - 15, tableTop, { align: "right" })
    y = tableTop + 10
    ...
    for (const item of items) {
      ...
      doc.text(String(item.quantity), pw - margin - 55, y)
      doc.text(`${currencySymbol}${item.rate.toFixed(2)}`, pw - margin - 35, y)
      doc.text(`${currencySymbol}${item.amount.toFixed(2)}`, pw - margin - 15, y, { align: "right" })
      y += 7
    }

    y += 5
    const tx = pw - margin - 55
    doc.setFont("helvetica", "normal")
    doc.setFontSize(9)
    doc.setTextColor(100, 116, 139)
    doc.text("Subtotal:", tx, y)
    doc.text(`${currencySymbol}${subtotal.toFixed(2)}`, pw - margin - 15, y, { align: "right" })
    y += 6
    if (taxRate > 0) { doc.text(`Tax (${taxRate}%):`, tx, y); doc.text(`${currencySymbol}${taxAmount.toFixed(2)}`, pw - margin - 15, y, { align: "right" }); y += 6 }
    if (discount > 0) { doc.text("Discount:", tx, y); doc.text(`-${currencySymbol}${discount.toFixed(2)}`, pw - margin - 15, y, { align: "right" }); y += 6 }
    if (shipping > 0) { doc.text("Shipping:", tx, y); doc.text(`${currencySymbol}${shipping.toFixed(2)}`, pw - margin - 15, y, { align: "right" }); y += 6 }
    doc.setDrawColor(5, 150, 105)
    doc.line(tx, y, pw - margin, y)
    y += 2
    doc.setFont("helvetica", "bold")
    doc.setFontSize(12)
    doc.setTextColor(5, 150, 105)
    doc.text("Total:", tx, y)
    doc.text(`${currencySymbol}${total.toFixed(2)}`, pw - margin - 15, y, { align: "right" })
    ...
    if (notes || terms) {
      ...
      doc.setFontSize(9)
      doc.setTextColor(26, 26, 46)
      if (notes) { doc.text("Notes:", margin, y); doc.setFont("helvetica", "normal"); doc.setTextColor(100, 116, 139); doc.setFontSize(8); doc.text(notes, margin, y + 5); y += 15 }
      if (terms) { doc.setFont("helvetica", "bold"); doc.setFontSize(9); doc.setTextColor(26, 26, 46); doc.text("Terms:", margin, y); doc.setFont("helvetica", "normal"); doc.setTextColor(100, 116, 139); doc.setFontSize(8); doc.text(terms, margin, y + 5) }
    }

    doc.save(`quotation-${quoteNum}.pdf`)
    ...
    <div className="mx-auto max-w-5xl space-y-8">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-teal-500/10"><FileText className="h-6 w-6 text-teal-500" /></div>
          <div><h1 className="text-2xl font-bold text-foreground">Quotation Generator</h1><p className="text-sm text-muted-foreground">Create professional quotations with PDF export</p></div>
        </div>
            ...
            <Card className="overflow-hidden p-0">
              <div className="bg-white p-10 dark:bg-gray-950">
                <div className="flex items-start justify-between border-b border-gray-200 pb-8 dark:border-gray-800">
                  <div>{logo && <img src={logo} alt="Logo" className="mb-4 max-h-16 object-contain" />}<h1 className="text-4xl font-extrabold tracking-tight text-teal-600 dark:text-teal-400">QUOTATION</h1><p className="mt-1 text-sm text-gray-500">{quoteNum}</p></div>
