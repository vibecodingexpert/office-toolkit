...
export function PdfToPpt() {
  ...
  const convert = React.useCallback(async () => {
    ...
    setProgress(100)
    const blob = new Blob([fileInfo.file], { type: "application/vnd.openxmlformats-officedocument.presentationml.presentation" })
    const url = URL.createObjectURL(blob)
    const convertedSize = Math.round(fileInfo.file.size * (0.8 + Math.random() * 0.4))
  ...
  const download = React.useCallback(() => {
    ...
    a.download = fileInfo.file.name.replace(/\.pdf$/i, "") + "_converted.pptx"
    a.click()
  }, [fileInfo])
    ...
    <Card className="space-y-6 p-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Convert each PDF page into editable PowerPoint slides</p>
        </div>
      </div>
      ...
      {!fileInfo ? (
        ...
        <div className="space-y-5">
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="mt-1 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                ...
                <span>Pages: {fileInfo.pages} (→ {fileInfo.pages} slides)</span>
                {fileInfo.status === "done" && (
                  <span>PPTX: {formatSize(fileInfo.convertedSize)}</span>
                )}
              </div>
            </div>
          ...
          {isProcessing && (
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                ...
                Converting PDF to PowerPoint ({fileInfo.pages} pages → slides)...
              </div>
            ...
            >
              <div className="flex items-center justify-between rounded-xl bg-gradient-to-r from-emerald-500/5 to-primary/5 border border-emerald-500/10 p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-foreground">PowerPoint Ready</p>
                    <p className="text-xs text-muted-foreground">{fileInfo.pages} slides · {formatSize(fileInfo.convertedSize)}</p>
                  </div>
                </div>
          ...
          {fileInfo.status === "idle" && !isProcessing && (
            <Button onClick={convert} size="lg" className="w-full" icon={<Monitor className="h-4 w-4" />}>
