...
export function WebpConverter() {
  ...
  const [targetFormat, setTargetFormat] = React.useState("image/png")
  ...
  return (
    <Card className="space-y-6 p-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-lg font-semibold">WebP Converter</h2>
          <p className="text-sm text-muted-foreground">Convert images to and from WebP</p>
        </div>
      </div>
      ...
      {!preview ? (
        <label className="flex cursor-pointer flex-col items-center gap-3 rounded-2xl border-2 border-dashed border-border bg-background p-8 text-center transition-all hover:border-primary/50 hover:bg-primary/[0.02]">
          <input type="file" accept="image/webp,image/png,image/jpeg" onChange={handleFile} className="hidden" />
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">Supports WebP, PNG, JPG</p>
          </div>
        </label>
      ) : (
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap gap-3">
            <Button onClick={handleConvert} loading={loading} icon={<Repeat className="h-4 w-4" />}>
              Convert
            </Button>
            {resultUrl && (
