...
export function JpgToWebp() {
  ...
  const handleDownload = () => {
    if (!result) return
    const link = document.createElement("a"); link.download = "converted.webp"; link.href = result; link.click()
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <div className="flex items-center gap-3"><ImageIcon className="h-6 w-6 text-teal-500" /><div><h1 className="text-2xl font-bold text-foreground">JPG to WebP</h1><p className="text-sm text-muted-foreground">Convert JPEG images to WebP format</p></div></div>
        <label className="cursor-pointer rounded-lg border border-input bg-background px-4 py-2 text-sm font-medium hover:bg-accent"><Upload className="mr-1 inline h-4 w-4" />Upload<input type="file" accept="image/jpeg" onChange={handleImage} className="hidden" /></label>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        {result && <Card className="flex items-center justify-center p-4"><img src={result} alt="WebP" className="max-h-80 rounded-lg" /></Card>}
      </div>
